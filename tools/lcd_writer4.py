#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fast LCD Writer prototype using NumPy RGB565 conversion."""
import curses
import fcntl
import mmap
import os
...
def rgb565_bytes(img):
    ...
    r = (rgb[:, :, 0].astype(np.uint16) * 31 // 255) << 11
    g = (rgb[:, :, 1].astype(np.uint16) * 63 // 255) << 5
    b = rgb[:, :, 2].astype(np.uint16) * 31 // 255
...
def main(stdscr):
    ...
    lines = [""]
    try:
        write_frame(mm, info, render(info["w"], info["h"], font, small, lines)); mm.flush()
        while True:
            ch = stdscr.get_wch()
