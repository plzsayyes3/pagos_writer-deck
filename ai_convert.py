...
- "mozc_local": mozc_emacs_helper経由でmozcの変換エンジンをGUI無しで直接
  ...
  Docker上のDebian trixie(実機と同じディストリ・同じemacs-mozc-binパッケージ・
  一般ユーザー権限)で実際に動作検証済み(2026-08-24)。「きょうはいいてんき
  ですね」→「今日はいい天気ですね」等、複数行・数字混じりでも正しく変換
  できることを確認した。実機(armhf)そのものでの最終確認はまだだが、
  高い確度で動くと見込んでいる。念のため実際に使う前に
  `python3 mozc_convert.py "てすと"` で一度動作確認すること。
